...
from database.recommendation_repository import get_candidate_players, fetch_all_competitions, fetch_all_teams, fetch_all_seasons

# Page Config handled by central entry point app.py

st.title("🔄 AI Player Recommendations")

# Load filter options from repository.
candidates = get_candidate_players()
player_names = sorted({c["player_name"] for c in candidates if c.get("player_name")})
...
with st.sidebar.expander("Advanced Filters", expanded=True):
    ...
    # Supported filters from the repository/service.
    club = st.selectbox(
        "🏟️ Club",
        [""] + sorted({c["team_name"] for c in candidates if c.get("team_name") and c["team_name"] != "N/A"}),
        key="club_filter",
    ) or None
    ...
    nationality = st.selectbox(
        "🌍 Nationality",
        [""] + sorted({c.get("nationality", "") for c in candidates if c.get("nationality")}),
